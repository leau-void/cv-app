...
const App = () => {
  const [general, setGeneral] = useState(defaultState.general);
  const [professional, setProfessional] = useState(defaultState.professional);
  const [education, setEducation] = useState(defaultState.education);

  const state = { general, professional, education };

  return (
    <div>
      <Header />
      <OuterForm {...{ state }} handlers={this.handlers} />
      <Preview {...{ state }} />
      <Footer />
    </div>
  );
